"""
same version as motion_audio_transformer_vocos_4.py but changed so that it imports motion data in the form of saved OSC messages that have been sent from Xsens MVN
"""
...
"""
mocap_data_path = "D:/data/mocap/Motion2Audio/stocos/fbx_50hz/"
...
mocap_data_path = "E:/data/mocap/Diane/Solos/ZHdK_10.10.2025/mvn_pkl/"
mocap_data_files = ["Diane_Take1.pkl"]
mocap_valid_ranges = [[466, 18947]]
...
mocap_pos_scale = 0.1
mocap_fps_orig = 100
mocap_fps = 50 # must be divideable by an integer value with mocap_fps_orig
...
load_audio_mels_stat = False
audio_mels_mean_file = "results/stat/latents_mean.pt"
audio_mels_std_file = "results/stat/latents_std.pt"
...
load_weights = False
save_weights = True
transformer_load_weights_path = "results/weights/transformer_weights_epoch_50"
epochs = 200
...
mocap_import_frame_skip = mocap_fps_orig // mocap_fps
...
def load_pkl(file_path):
    ...
    sensor_ids = file_data["sensor_ids"]
    sensor_data = file_data["sensor_values"]
    ...
    for unique_sensor_id in unique_sensor_ids:
        ...
        sensor_id_indices =  [i for i, v in enumerate(sensor_ids) if v == unique_sensor_id]
        
        print("unique_sensor_id ", unique_sensor_id, " count ", len(sensor_id_indices))

        sensor_values_per_sensor_id = [sensor_data[i] for i in sensor_id_indices]
        
        print("sensor_values_per_sensor_id count ", len(sensor_values_per_sensor_id))
        
        sensor_values_per_sensor_id = np.array(sensor_values_per_sensor_id)

        mocap_motion_data[unique_sensor_id] = sensor_values_per_sensor_id
        
    return mocap_motion_data

def change_mocap_fps(motion_data, orig_fps, target_fps):
    
    frame_skip = orig_fps // target_fps
    
    sensor_ids = list(motion_data.keys())
    
    print("sensor_ids ", sensor_ids)
    
    for sensor_id in sensor_ids:
        motion_data[sensor_id] = motion_data[sensor_id][::frame_skip, ...]
        
    return motion_data
    

mocap_motion_data = load_pkl(mocap_data_path + mocap_data_files[0])
mocap_motion_data = change_mocap_fps(mocap_motion_data, mocap_fps_orig, mocap_fps)


mocap_duration = mocap_motion_data["/mocap/0/joint/rot_world"].shape[0] // mocap_fps
mocap_duration

"""
# code to load mocap data stored as osc messages in an exported python dictionary

with open("C:/Users/dbisig/Projects/Premiere/Software_Git2/MotionUtilities/SensorRecorder/recordings/Diane_Take1.pkl", "rb") as f:
    data = pickle.load(f)
...
# compute and normalise mel spectra

if load_audio_mels_stat == True:
    audio_mels_mean = torch.load(audio_mels_mean_file)
    audio_mels_std = torch.load(audio_mels_std_file)
    
    audio_mels_mean = audio_mels_mean.to(device)
    audio_mels_std = audio_mels_std.to(device)
    ...
    with torch.no_grad():
        ...
        for audio_waveform_data in audio_all_data: 
            ...
            audio_mels_all.append(audio_mels)
            
        audio_mels_all = torch.cat(audio_mels_all, dim=2)
        
        audio_mels_mean = torch.mean(audio_mels_all, dim=2, keepdim=True)
        audio_mels_std = torch.std(audio_mels_all, dim=2, keepdim=True)
        
        print("audio_mels_mean s ", audio_mels_mean.shape)
        print("audio_mels_std s ", audio_mels_std.shape)
        
        torch.save(audio_mels_mean.detach().cpu(), audio_mels_mean_file)
        torch.save(audio_mels_std.detach().cpu(), audio_mels_std_file)
...
for sI in range(len(mocap_all_data)):
    ...
    with torch.no_grad():
        ...
        for mfI in range(3, mocap_frame_count - mocap_input_output_seq_length - 10, mocap_frame_incr):
            ...
            audio_mels_norm = (audio_mels_excerpt - audio_mels_mean) / (audio_mels_std + 1e-8)
            
            #print("audio_mels_excerpt s ", audio_mels_excerpt.shape)
            
            #print("mfI ", mfI, " audio_mels_norm s ", audio_mels_norm.shape)
            
            audio_mels_norm = audio_mels_norm.permute((0, 2, 1))
            
            #print("mfI ", mfI, " audio_mels_norm 2 s ", audio_mels_norm.shape)
            
            X_audio.append(audio_mels_norm.cpu())
            ...
            for tsI in range(mocap_output_seq_length):
                ...
                #print("mfI2 ", mfI2, " audio_mels_norm s ", audio_mels_norm.shape)
                
                audio_mels_norm = audio_mels_norm.permute((0, 2, 1))
                
                #print("mfI2 ", mfI2, " audio_mels_norm 2 s ", audio_mels_norm.shape)
                
                y_audio_grouped.append(audio_mels_norm.cpu())
...
print("X_mocap[0] s ", X_mocap[0].shape)
print("X_audio[0] s ", X_audio[0].shape)
print("Y_audio[0] s ", Y_audio[0].shape)
...
def loss(y_mels_norm, yhat_mels_norm):
    ...
    _audio_mel_loss = audio_mel_loss(y_mels_norm, yhat_mels_norm)
    
    # convert audio mels to audio waveforms
    y_mels_norm = y_mels_norm.permute((0, 2, 1))
    yhat_mels_norm = yhat_mels_norm.permute((0, 2, 1))
    
    #print("y_mels_norm 2 s ", y_mels_norm.shape, " yhat_mels_norm 2 s ", yhat_mels_norm.shape)

    y_mels = y_mels_norm * audio_mels_std + audio_mels_mean
    yhat_mels = yhat_mels_norm * audio_mels_std + audio_mels_mean
    ...
    _total_loss = 0.0
    _total_loss += _audio_mel_loss * 0.5
...
def create_gen_audio(mocap_data, waveform_data, mocap_start_frame_index, mocap_frame_count, file_name):
    ...
    mocap_excerpt = mocap_data[mocap_start_frame_index:mocap_end_frame_index, ...].to(device)
    mocap_excerpt_norm = (mocap_excerpt - mocap_mean) / (mocap_std + 1e-8)
    mocap_excerpt_norm = mocap_excerpt_norm.unsqueeze(0)
    ...
    audio_waveform_excerpt = waveform_data[audio_waveform_start_sample_index:audio_waveform_end_sample_index]
    audio_waveform_excerpt = audio_waveform_excerpt.unsqueeze(0).to(device)
    ...
    audio_mels_excerpt_norm = (audio_mels_excerpt - audio_mels_mean) / (audio_mels_std + 1e-8)
    
    #print("audio_mels_excerpt_norm s ", audio_mels_excerpt_norm.shape)
    
    audio_mels_excerpt_norm = audio_mels_excerpt_norm.permute((0, 2, 1))
    
    #print("audio_mels_excerpt_norm 2 s ", audio_mels_excerpt_norm.shape)
    
    x_mocap = mocap_excerpt_norm[:, :mocap_input_seq_length, ...]
    
    #print("x_mocap s ", x_mocap.shape)
    
    x_audio = audio_mels_excerpt_norm
    ...
    yhat_audio_mels_norm = yhat_audio
    
    #print("yhat_audio_mels_norm s ", yhat_audio_mels_norm.shape)
    
    yhat_audio_mels_norm = yhat_audio_mels_norm.permute((0, 2, 1))
    
    #print("yhat_audio_mels_norm 2 s ", yhat_audio_mels_norm.shape)
    
    yhat_audio_mels = yhat_audio_mels_norm * audio_mels_std + audio_mels_mean
    ...
    for mFI in range(1, mocap_frame_count - mocap_input_seq_length):  
        ...
        x_mocap = mocap_excerpt_norm[:, mFI:mFI + mocap_input_seq_length, ...]
        ...
        yhat_audio_mels_norm = yhat_audio.detach()
...
test_mocap_data_file = "E:/data/mocap/Diane/Solos/ZHdK_10.10.2025/fbx_50hz/trial-002.fbx"
test_mocap_valid_ranges = [[466, 18947]]

"""
test_mocap_data_file = "E:/data/mocap/Eleni/fbx_50hz/Eline_Session-001.fbx"
