"""
same as motion_audio_transformer_vocos but the dataset contains waveforms instead of mel spectra and the mel spectra are calculated on the fly during training
"""
...
audio_mels_mean_file = "results/stat/mels_mean.pt"
audio_mels_std_file = "results/stat/mels_std.pt"
...
load_weights = True
save_weights = False
transformer_load_weights_path = "results/weights/transformer_weights_epoch_100"
epochs = 100
...
pos_encoding_max_length = max(mocap_input_seq_length, audio_mel_input_seq_length)
...
for sI in range(len(mocap_all_data)):
    ...
    for mfI in range(3, mocap_frame_count - mocap_input_output_seq_length - 10, mocap_frame_incr):
        ...
        audio_waveform_end = audio_waveform_start + mocap_input_output_seq_length * audio_samples_per_mocap_frame
        ...
        X_audio.append(audio_waveform_excerpt.cpu())
...
class SequenceDataset(Dataset):
    def __init__(self, X_mocap, X_audio):
    ...
    def __getitem__(self, idx):
        return self.X_mocap[idx, ...], self.X_audio[idx, ...]

full_dataset = SequenceDataset(X_mocap, X_audio)

x_item_mocap, x_item_audio = full_dataset[0]
...
x_batch_mocap, x_batch_audio = next(iter(train_loader))
...
x_mocap_batch, x_audio_batch = next(iter(train_loader))

transformer_mocap_input = x_mocap_batch[:, :mocap_input_seq_length, ...].to(device)

x_audio_waveform = x_audio_batch[:, :audio_waveform_input_seq_length].to(device)
y_audio_waveform = x_audio_batch[:, audio_samples_per_mocap_frame:audio_waveform_input_seq_length + audio_samples_per_mocap_frame].to(device)

x_audio_mels = vocos.feature_extractor(x_audio_waveform)
y_audio_mels = vocos.feature_extractor(y_audio_waveform)

print("x_audio_waveform s ", x_audio_waveform.shape)
print("x_audio_mels s ", x_audio_mels.shape)

x_audio_mels_norm = (x_audio_mels - audio_mels_mean) / (audio_mels_std + 1e-8)
y_audio_mels_norm = (y_audio_mels - audio_mels_mean) / (audio_mels_std + 1e-8)

print("x_audio_mels_norm s ", x_audio_mels_norm.shape)

transformer_audio_input = x_audio_mels_norm.permute((0, 2, 1))
transformer_audio_target = y_audio_mels_norm.permute((0, 2, 1))
...
def train_step(x_mocap, x_audio):
    ...
    for mfI in range(1, mocap_output_seq_length - 1):
        ...
        y_audio_waveform = x_audio_batch[:, audio_samples_per_mocap_frame * (mfI + 1):audio_waveform_input_seq_length + audio_samples_per_mocap_frame * (mfI + 1)].to(device)
        y_audio_mels = vocos.feature_extractor(y_audio_waveform)
        y_audio_mels_norm = (y_audio_mels - audio_mels_mean) / (audio_mels_std + 1e-8)
        transformer_audio_target = y_audio_mels_norm.permute((0, 2, 1))
...
_loss = train_step(x_mocap_batch, x_audio_batch)
"""

@torch.no_grad()
def test_step(x_mocap, x_audio):
...
def train(train_dataloader, test_dataloader, epochs):
    ...
    for epoch in range(epochs):
        ...
        for train_batch in train_dataloader:
            ...
            _loss = train_step(x_mocap, x_audio)
        ...
        for test_batch in test_dataloader:
            ...
            _loss = test_step(x_mocap, x_audio)
...
def create_vocos_audio(waveform_data, mocap_start_frame_index, mocap_frame_count, file_name):
    ...
    for aSI in range(0, audio_waveform_excerpt_sample_count - audio_waveform_input_seq_length, audio_samples_per_mocap_frame):
        ...
        audio_mels_norm = (audio_mels - audio_mels_mean) / (audio_mels_std + 1e-8) 
        
        audio_mels_2 = audio_mels_norm * audio_mels_std + audio_mels_mean
        
        #print("audio_mels_2 s ", audio_mels.shape)
        
        audio_waveform_2 = vocos.decode(audio_mels_2)
...
def create_gen_audio(mocap_data, waveform_data, mocap_start_frame_index, mocap_frame_count, file_name):
    ...
    audio_mels_norm_data = (audio_mels_data - audio_mels_mean) / (audio_mels_std + 1e-8)
    
    #print("audio_mels_norm_data s ", audio_mels_norm_data.shape)
    ...
    x_audio = audio_mels_norm_data.permute((0, 2, 1))
    ...
    gen_audio_mels_norm = yhat_audio.detach().permute(0, 2, 1)
    
    #print("gen_audio_mels_norm s ", gen_audio_mels_norm.shape)
    
    gen_audio_mels = gen_audio_mels_norm * audio_mels_std + audio_mels_mean
    
    #print("gen_audio_mels s ", gen_audio_mels.shape)

    gen_waveform = vocos.decode(gen_audio_mels)
    
    #print("gen_waveform s ", gen_waveform.shape)
    
    gen_waveform_window = gen_waveform.reshape(-1)[-audio_window_length:]
    gen_waveform_window = gen_waveform_window.detach().cpu()
    
    #print("gen_waveform_window s ", gen_waveform_window.shape)
    
    gen_audio_waveform[:audio_window_length] += gen_waveform_window * audio_window_env
    ...
    for mFI in range(1, mocap_frame_count - mocap_input_seq_length):  
        ...
        gen_audio_waveform[aSI:aSI+ audio_window_length] += gen_waveform_window * audio_window_env
...
create_gen_audio(test_mocap_data, test_audio_data, 60 * mocap_fps, 60 * mocap_fps, "gen_audio.wav")
