...
audio_data_path = "E:/Data/audio/Diane/44khz/"
...
if load_audio_latents_stat == True:
    ...
    with torch.no_grad():
        ...
        for audio_waveform_data in audio_all_data: 
            audio_latents = encdec.encode(audio_waveform_data[0])
...
for sI in range(len(mocap_all_data)):
    ...
    with torch.no_grad():
        ...
        for mfI in range(3, mocap_frame_count - mocap_input_output_seq_length - 10, mocap_frame_incr):
            ...
            audio_latents_excerpt = encdec.encode(audio_waveform_excerpt[0])
            ...
            X_audio.append(audio_latents_excerpt_norm.cpu())
            ...
            for tsI in range(mocap_output_seq_length):
                ...
                y_audio_grouped.append(audio_latents_excerpt_norm.cpu())
...
@torch.no_grad()
def create_music2latent_audio(waveform_data, mocap_start_frame_index, mocap_frame_count, file_name):
    ...
    for aSI in range(0, audio_waveform_excerpt_sample_count - audio_waveform_input_seq_length, audio_samples_per_mocap_frame):
        ...
        audio_waveform = audio_waveform.to(torch.float16)
        ...
        audio_latents = encdec.encode(audio_waveform[0])
        audio_latents = audio_latents.to(torch.float32)
        ...
        audio_latents_norm = audio_latents_norm.permute((0, 2, 1))
        
        #print("audio_latents_norm 2 s ", audio_latents_norm.shape)
        
        audio_latents_norm = audio_latents_norm.permute((0, 2, 1))
        
        #print("audio_latents_norm 3 s ", audio_latents_norm.shape)
        ...
        audio_waveform_2 = encdec.decode(audio_latents)
        
        #print("audio_waveform_2 s ", audio_waveform_2.shape)
        
        audio_waveform_2 = audio_waveform_2.to(torch.float16)
...
create_music2latent_audio(audio_all_data[0][0], 60 * mocap_fps, 10 * mocap_fps, "music2latent_audio.wav")
...
def create_gen_audio(mocap_data, waveform_data, mocap_start_frame_index, mocap_frame_count, file_name):
    ...
    audio_waveform_data = audio_waveform_data.to(torch.float16)
    
    #print("audio_waveform_data s ", audio_waveform_data.shape)
    
    audio_waveform_data = audio_waveform_data.reshape((1, audio_waveform_input_seq_length))
    ...
    yhat_audio_latents_norm = yhat_audio.detach()
    
    #print("yhat_audio_latents_norm s ", yhat_audio_latents_norm.shape)
    
    yhat_audio_latents_norm = yhat_audio_latents_norm.permute((0, 2, 1))
    
    #print("yhat_audio_latents_norm 2 s ", yhat_audio_latents_norm.shape)
    ...
    yhat_audio_latents = yhat_audio_latents.to(torch.float16)
    
    yhat_audio_waveform = encdec.decode(yhat_audio_latents)
    
    yhat_audio_waveform = yhat_audio_waveform.to(torch.float32)
...
for test_mocap_start_time in test_mocap_start_times:
    ...
    create_music2latent_audio(test_audio_data, test_mocap_start_time * mocap_fps, test_mocap_duration * mocap_fps, "results/audio/music2latent_audio_{}-{}.wav".format(test_mocap_start_time, (test_mocap_start_time + test_mocap_duration)))
    create_gen_audio(test_mocap_data, test_audio_data, test_mocap_start_time * mocap_fps, test_mocap_duration * mocap_fps, "results/audio/gen_audio_{}-{}_epoch_{}_orig.wav".format(test_mocap_start_time, (test_mocap_start_time + test_mocap_duration), epochs))
