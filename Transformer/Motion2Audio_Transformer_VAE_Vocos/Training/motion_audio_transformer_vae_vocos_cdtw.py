"""
same as motion_audio_transformer_vae_vocos_4_2.py but with the option to temporally align the mocap recordings using continuous dynamic time warping
"""
...
# imports for continuous dynamic time warping
...
mocap_time_align = True # temporally align all mocap recordings with the first one
mocap_pos_scale = 1.0
mocap_fps = 60
...
"""
Temporally Align Mocap Recodings
"""

# Process Mocap Sequences (convert float64 to float32)
for mI in range(len(mocap_all_data)):
    
    # cast float64 to float32
    mocap_all_data[mI]["motion"]["rot_local"] = mocap_all_data[mI]["motion"]["rot_local"].astype(np.float32)
    mocap_all_data[mI]["motion"]["pos_local"] = mocap_all_data[mI]["motion"]["pos_local"].astype(np.float32)
...
def quat_slerp(q0, q1, t):
    ...
    t: array broadcastable to q0[..., 0]
    returns: SLERP(q0, q1, t) with same shape as q0
    """
    ...
    negate = dot < 0.0
    q1 = np.where(negate, -q1, q1)
    dot = np.where(negate, -dot, dot)
    ...
    # Make t have same leading shape as q0[..., 0]
    t = np.asarray(t, dtype=np.float32)
    # t should be broadcastable to q0[..., 0]
    # Add last singleton for quaternion dimension
    while t.ndim < q0.ndim - 1:
        t = t[..., None]
    t = t[..., None]  # final shape (..., 1) to match last dim=4

    small = sin_theta_0 < EPS

    # Linear fallback
    lerp = quat_normalize((1.0 - t) * q0 + t * q1)
    ...
    return np.where(small, lerp, slerp)

# ---------- Continuous DTW warping for (T, J, C) with C=3 or 4 ----------

def align_to_reference_timeseries_continuous(X, Ref):
    """
    Align time series X (T_x, J, C) to Ref (T_ref, J, C) using DTW,
    then perform continuous warping via interpolation.

    - If C == 3: positions (x, y, z), linear interpolation.
    - If C == 4: quaternions (w, x, y, z), SLERP per joint.

    Returns X_warped with shape (T_ref, J, C).
    """
    X = np.asarray(X, dtype=np.float32)
    Ref = np.asarray(Ref, dtype=np.float32)

    assert X.ndim == 3 and Ref.ndim == 3, "Expect (T, J, C)"
    T_x, J_x, C_x = X.shape
    T_ref, J_ref, C_ref = Ref.shape
    assert J_x == J_ref, "joint_count mismatch"
    assert C_x == C_ref, "channel count mismatch (3 or 4)"
    assert C_x in (3, 4), "C must be 3 (positions) or 4 (quaternions)"
    J, C = J_x, C_x

    # 1) DTW on flattened frames (time x features). [web:18]
    X_flat = X.reshape(T_x, J * C)
    Ref_flat = Ref.reshape(T_ref, J * C)

    res = dtw(X_flat, Ref_flat, dist_only=False)
    path = res.path  # (L, 2): (idx_X, idx_Ref)

    x_idx = path[:, 0].astype(np.float32)
    ref_idx = path[:, 1].astype(np.float32)

    # 2) Mapping ref_index -> fractional x_index
    x_for_ref = np.full(T_ref, np.nan, dtype=np.float32)
    for j in range(T_ref):
        mask = (ref_idx == j)
        if np.any(mask):
            x_for_ref[j] = x_idx[mask].mean()

    # Fill gaps
    known = ~np.isnan(x_for_ref)
    if not known[0]:
        first_known = np.argmax(known)
        x_for_ref[:first_known] = x_for_ref[first_known]
    if not known[-1]:
        last_known = T_ref - 1 - np.argmax(known[::-1])
        x_for_ref[last_known+1:] = x_for_ref[last_known]

    known = ~np.isnan(x_for_ref)
    x_for_ref[~known] = np.interp(
        np.flatnonzero(~known),
        np.flatnonzero(known),
        x_for_ref[known],
    )

    x_for_ref = np.clip(x_for_ref, 0, T_x - 1)
    x_for_ref = np.maximum.accumulate(x_for_ref)

    # 3) Interpolation
    X_warped = np.empty((T_ref, J, C), dtype=np.float32)

    if C == 3:
        # Positions: linear interpolation per joint and coordinate. [web:18]
        t_x = np.arange(T_x, dtype=np.float32)
        for j in range(J):
            for c in range(C):
                X_warped[:, j, c] = np.interp(x_for_ref, t_x, X[:, j, c])

    else:  # C == 4, quaternions
        X_norm = quat_normalize(X)  # (T_x, J, 4)

        t0 = np.floor(x_for_ref).astype(int)
        t1 = np.ceil(x_for_ref).astype(int)
        t1 = np.clip(t1, 0, T_x - 1)
        alpha = (x_for_ref - t0).astype(np.float32)  # (T_ref,)

        # Gather q0, q1: (T_ref, J, 4)
        q0 = X_norm[t0]
        q1 = X_norm[t1]

        # Broadcast alpha over (T_ref, J)
        # shape: (T_ref, 1) -> (T_ref, J)
        alpha_mat = alpha[:, None] * np.ones((1, J))
        q_interp = quat_slerp(q0, q1, alpha_mat)  # (T_ref, J, 4)

        X_warped[:] = q_interp

    return X_warped

if mocap_time_align == True:

    mocap_pos_seq_ref = mocap_all_data[0]["motion"]["pos_local"]
    mocap_rot_seq_ref = mocap_all_data[0]["motion"]["rot_local"]
    
    for mI in range(1, len(mocap_all_data)):
        mocap_pos_seq = mocap_all_data[mI]["motion"]["pos_local"]
        mocap_rot_seq = mocap_all_data[mI]["motion"]["rot_local"]
        
        mocap_pos_seq_aligned = align_to_reference_timeseries_continuous(mocap_pos_seq, mocap_pos_seq_ref)
        mocap_rot_seq_aligned = align_to_reference_timeseries_continuous(mocap_rot_seq, mocap_rot_seq_ref)
    
        mocap_all_data[mI]["motion"]["pos_local"] = mocap_pos_seq_aligned
        mocap_all_data[mI]["motion"]["rot_local"] = mocap_rot_seq_aligned 
