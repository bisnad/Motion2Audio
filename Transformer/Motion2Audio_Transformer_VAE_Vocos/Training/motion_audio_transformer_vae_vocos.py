...
audio_channels = 1
audio_waveform_length_per_mocap_frame = audio_sample_rate // mocap_fps

audio_latents_transformer_input_seq_length = mocap_input_seq_length
audio_latents_transformer_output_seq_length = 10 # for non teacher forcing-based training
audio_latents_transformer_input_output_seq_length = audio_latents_transformer_input_seq_length + audio_latents_transformer_output_seq_length

audio_waveform_vae_length = None # automatically calculated
audio_waveform_transformer_input_length = None # automatically calculated
audio_waveform_transformer_output_length = None # automatically calculated
audio_waveform_transformer_input_output_length = None # automatically calculated

audio_mels_vae_length = 9
audio_mels_input_length = audio_mels_vae_length * audio_latents_transformer_input_seq_length
audio_mels_output_length = audio_mels_vae_length * audio_latents_transformer_output_seq_length
audio_mels_input_output_seq_length = audio_mels_input_length + audio_mels_output_length

audio_window_increment = audio_sample_rate // mocap_fps
audio_window_offset = audio_window_increment
...
encoder_weights_file = "../../AudioTransform/vae_vocos_v2/results_audio_vae_vocos_cnn_DianeFather/weights/encoder_weights_epoch_400"
decoder_weights_file = "../../AudioTransform/vae_vocos_v2/results_audio_vae_vocos_cnn_DianeFather/weights/decoder_weights_epoch_400"
...
load_weights = True
save_weights = True
transformer_load_weights_path = "results/weights/transformer_weights_epoch_20"
epochs = 320
...
# determine number of mel filter bands
dummy_mels = vocos.feature_extractor(torch.rand(size=(1, audio_sample_rate), dtype=torch.float32).to(device))
audio_mel_filter_count = dummy_mels.shape[1]      

# determine audio_waveform_vae_length
dummy_waveform = vocos.decode((torch.rand(1, audio_mel_filter_count, audio_mels_vae_length)).to(device))
audio_waveform_vae_length = dummy_waveform.shape[1]

# determine audio_waveform_transformer_input_length
dummy_waveform = vocos.decode((torch.rand(1, audio_mel_filter_count, audio_mels_input_length)).to(device))
audio_waveform_transformer_input_length = dummy_waveform.shape[1]

# determine audio_waveform_transformer_output_length
dummy_waveform = vocos.decode((torch.rand(1, audio_mel_filter_count, audio_mels_output_length)).to(device))
audio_waveform_transformer_output_length = dummy_waveform.shape[1]

# determine audio_waveform_transformer_input_output_seq_length
dummy_waveform = vocos.decode((torch.rand(1, audio_mel_filter_count, audio_mels_input_length + audio_mels_output_length)).to(device))
audio_waveform_transformer_input_output_length = dummy_waveform.shape[1]
...
encoder = Encoder(latent_dim, audio_mels_vae_length, audio_mel_filter_count, vae_conv_channel_counts, vae_conv_kernel_size, vae_dense_layer_sizes).to(device)
...
audio_encoder_in = torch.zeros((batch_size, 1, audio_mel_filter_count, audio_mels_vae_length)).to(device)
...
decoder = Decoder(latent_dim, audio_mels_vae_length, audio_mel_filter_count, vae_conv_channel_counts_reversed, vae_conv_kernel_size, vae_dense_layer_sizes_reversed).to(device)
...
if load_vocos_stat == True:
    ...
    with torch.no_grad():
        ...
        for audio_data in audio_all_data:    
            ...
            for asI in range(0, audio_sample_count - audio_waveform_transformer_input_output_length, audio_window_increment):
                
                audio_excerpt = audio_data[:, asI:asI+audio_waveform_transformer_input_output_length].to(device)
                ...
                # regroup mels
                audio_mels_regrouped = audio_mels.reshape(1, 1, audio_mel_filter_count, audio_mels_input_output_seq_length // audio_mels_vae_length, audio_mels_vae_length)
                audio_mels_regrouped = audio_mels_regrouped.permute(0, 3, 1, 2, 4) 
                audio_mels_regrouped = audio_mels_regrouped.reshape(-1, 1, audio_mel_filter_count, audio_mels_vae_length)
                                                  
                #print("audio_mels_regrouped s ", audio_mels_regrouped.shape)
                
                encoder_out_mu, encoder_out_std = encoder(audio_mels_regrouped)
    ...
    #print("vocos_latents_all_data s ", vocos_latents_all_data.shape)
...
with torch.no_grad():
    for sI in range(len(mocap_all_data)):
        ...
        for mfI in range(3, mocap_frame_count - mocap_input_seq_length - 2 - non_teacher_forcing_step_count, mocap_frame_incr):
            ...
            # audio sequence part
            audio_waveform_excerpt_start = mfI * audio_waveform_length_per_mocap_frame - audio_waveform_vae_length
            audio_waveform_excerpt_end = audio_waveform_excerpt_start + audio_waveform_transformer_input_output_length
            #print("mocap frame ", mfI, " waveform start ", audio_waveform_excerpt_start, " waveform end ", audio_waveform_excerpt_end)
            
            audio_waveform_excerpt = audio_data[audio_waveform_excerpt_start:audio_waveform_excerpt_end].to(device)
            #print("audio_waveform_excerpt s ", audio_waveform_excerpt.shape)
            
            if audio_waveform_excerpt.shape[0] < audio_waveform_transformer_input_output_length:
                
                #print("audio waveform excerpt to short")
            ...
            # regroup mels
            audio_mels_regrouped = audio_mels_excerpt.reshape(1, 1, audio_mel_filter_count, audio_mels_input_output_seq_length // audio_mels_vae_length, audio_mels_vae_length)
            audio_mels_regrouped = audio_mels_regrouped.permute(0, 3, 1, 2, 4) 
            audio_mels_regrouped = audio_mels_regrouped.reshape(-1, 1, audio_mel_filter_count, audio_mels_vae_length)
            #print("audio_mels_regrouped s ", audio_mels_regrouped.shape)
...
y_batch_audio = y_batch_audio[:, :audio_latents_transformer_input_seq_length, :].to(device)
...
def loss(y_latents_norm, yhat_latents_norm):
    ...
    y_latents = y_latents_norm * vocos_latents_std + vocos_latents_mean
    yhat_latents = yhat_latents_norm * vocos_latents_std + vocos_latents_mean
    ...
    # regroup mels
    y_mels = y_mels_regrouped.reshape(batch_size, -1, 1, audio_mel_filter_count, audio_mels_vae_length)
    yhat_mels = yhat_mels_regrouped.reshape(batch_size, -1, 1, audio_mel_filter_count, audio_mels_vae_length)
...
def train_step(x_mocap, y_audio):
    ...
    y_audio_input = y_audio[:, :audio_latents_transformer_input_seq_length, :]
    y_audio_target = y_audio[:, 1:audio_latents_transformer_input_seq_length+1, :] # shift target by one so we predict next token
    ...
    step_loss = loss(y_audio_target, yhat_audio_target) 
    ...
    for i in range(non_teacher_forcing_step_count):
        ...
        x_mocap_input = x_mocap[:, i+1:mocap_input_seq_length+i+1, :]
        y_audio_target = y_audio[:, i+1:audio_latents_transformer_input_seq_length+i+1, :] # shift target by one so we predict next token
        yhat_audio_target = transformer(x_mocap_input, y_audio_input)
...
audio_window_env = torch.hann_window(audio_waveform_length_per_mocap_frame)
...
def create_ref_audio_vae(waveform_data, mocap_start_frame_index, mocap_frame_count, file_name):
    
    with torch.no_grad():
        
        #print("waveform_data s ", waveform_data.shape)
        
        waveform_start_index = mocap_start_frame_index * audio_waveform_length_per_mocap_frame
        waveform_end_index = waveform_start_index + mocap_frame_count * audio_waveform_length_per_mocap_frame
        waveform_start_index = waveform_start_index - audio_waveform_vae_length
        ...
        for asI in range(0, audio_sample_count - audio_waveform_transformer_input_length, audio_waveform_vae_length):
            
            #print("asI ", asI)
            
            audio_window = waveform_data[waveform_start_index + asI:waveform_start_index + asI + audio_waveform_transformer_input_length].to(device)
            ...
            # regroup mels
            audio_mels_regrouped = audio_mels.reshape(1, 1, audio_mel_filter_count, audio_mels_input_length // audio_mels_vae_length, audio_mels_vae_length)
            
            #print("audio_mels_regrouped 2 s ", audio_mels_regrouped.shape)
            
            audio_mels_regrouped = audio_mels_regrouped.permute(0, 3, 1, 2, 4) 
            
            #print("audio_mels_regrouped 3 s ", audio_mels_regrouped.shape)
            
            audio_mels_regrouped = audio_mels_regrouped.reshape(-1, 1, audio_mel_filter_count, audio_mels_vae_length)
                                                  
            #print("audio_mels_regrouped 4 s ", audio_mels_regrouped.shape)
            ...
            # TODO: from here: regroup again the audio decoder output so it can be passed into vocos decode
            
            audio_decoder_out_regrouped = audio_decoder_out.permute(1, 2, 0, 3) 
            
            #print("audio_decoder_out_regrouped s ", audio_decoder_out_regrouped.shape)
            
            audio_decoder_out_regrouped = audio_decoder_out_regrouped.reshape(1, audio_mel_filter_count, -1)
            
            #print("audio_decoder_out_regrouped 2 s ", audio_decoder_out_regrouped.shape)
            
            audio_decoder_out_regrouped = audio_decoder_out_regrouped[:, :, -audio_mels_vae_length:]
            
            #print("audio_decoder_out_regrouped 3 s ", audio_decoder_out_regrouped.shape)
            
            rec_mels.append(audio_decoder_out_regrouped.detach())
            
            #voc_audio = vocos.decode(audio_decoder_out_regrouped.squeeze(1))
        ...
        rec_mels = torch.cat(rec_mels, dim=-1)
        
        #print("rec_mels s ", rec_mels.shape)
        
        rec_audio = vocos.decode(rec_mels).detach().cpu()

    torchaudio.save(file_name, rec_audio, audio_sample_rate)

create_ref_audio_vae(audio_all_data[0][0], 60 * mocap_fps, 10 * mocap_fps, "ref_audio_vae.wav")
...
def create_pred_audio_single(mocap_data, audio_data, mocap_start_frame_index, file_name):
    ...
    with torch.no_grad():
        ...
        x_mocap_norm =  torch.tensor(mocap_excerpt_norm, dtype=torch.float32).unsqueeze(0).to(device)
        #print("x_mocap_norm s ", x_mocap_norm.shape)
        
        # get initial audio excerpt
        audio_waveform_excerpt_start = mocap_start_frame_index * audio_waveform_length_per_mocap_frame - audio_waveform_vae_length
        audio_waveform_excerpt_end = audio_waveform_excerpt_start + audio_waveform_transformer_input_length
        
        audio_waveform_excerpt = audio_data[audio_waveform_excerpt_start:audio_waveform_excerpt_end].to(device)
        #print("audio_waveform_excerpt s ", audio_waveform_excerpt.shape)
        
        # convert initial audio excerpt to vocos latents
        audio_mels_excerpt = vocos.feature_extractor(audio_waveform_excerpt.reshape(1, 1, -1))
        #print("audio_mels_excerpt s ", audio_mels_excerpt.shape)
        
        # regroup mels
        audio_mels_regrouped = audio_mels_excerpt.reshape(1, 1, audio_mel_filter_count, audio_mels_input_length // audio_mels_vae_length, audio_mels_vae_length)
        ...
        y_audio_latents_norm = audio_latents_norm.unsqueeze(0)
        #print("y_audio_latents_norm s ", y_audio_latents_norm.shape)
        
        yhat_audio_latents_norm  = transformer(x_mocap_norm, y_audio_latents_norm)
        
        #print("yhat_audio_latents_norm s ", yhat_audio_latents_norm.shape)
        
        yhat_audio_latents = yhat_audio_latents_norm.squeeze(0) * vocos_latents_std + vocos_latents_mean
    
        #print("yhat_audio_latents s ", yhat_audio_latents.shape)
        
        audio_decoder_out = decoder(yhat_audio_latents)
        ...
        gen_mels = audio_decoder_out_regrouped
        
        print("gen_mels s ", gen_mels.shape)
        
        gen_audio = vocos.decode(gen_mels).detach().cpu()
        
        print("gen_audio s ", gen_audio.shape)
        
    torchaudio.save(file_name, gen_audio, audio_sample_rate)
        
        
    transformer.train()
    
"""
def create_pred_audio_single_2(mocap_data, audio_data, mocap_start_frame_index, file_name):
    ...
    with torch.no_grad():
        ...
        file_name = "gen_audio_single_{}.wav".format(0)
        
        torchaudio.save(file_name, gen_audio, audio_sample_rate)
        
        for mfI in range(1, 1000):
        
            print("mfI ", mfI)    
        
            # create mocap excerpt
            mocap_excerpt = mocap_data[mocap_start_frame_index + mfI:mocap_start_frame_index + mfI + mocap_input_seq_length, :]
            #print("mocap_excerpt s ", mocap_excerpt.shape)
            ...
            # create audio excerpt
            #print("y_audio_latents_norm s ", y_audio_latents_norm.shape)
            #print("yhat_audio_latents_norm s ", yhat_audio_latents_norm.shape)
            
            y_audio_latents_norm = torch.cat([y_audio_latents_norm[:, 1:, :].detach(), yhat_audio_latents_norm[:, -1:, :].detach()], axis=1)
            
            #print("y_audio_latents_norm 2 s ", y_audio_latents_norm.shape)
            ...
            if mfI % 40 == 0:
                file_name = "gen_audio_single_{}.wav".format(mfI)
...
create_pred_audio_single(test_mocap_data, test_audio_data, 60 * mocap_fps, "results/audio/gen_audio_single_epoch_{}.wav".format(epochs))
#create_pred_audio_single_2(test_mocap_data, test_audio_data, 60 * mocap_fps, "gen_audio_single.wav")
...
def create_pred_audio(mocap_data, audio_data, mocap_start_frame_index, mocap_frame_count, file_name):
    ...
    with torch.no_grad():
        ...
        gen_mels.append(audio_decoder_out_regrouped.detach())
        
        for mfI in range(1, mocap_frame_count):
            
            #print("mfI ", mfI)
            
            # create mocap input sequence
            mocap_excerpt = mocap_data[mocap_start_frame_index + mfI:mocap_start_frame_index + mocap_input_seq_length + mfI, :]
            ...
            # create audio input sequence
            #print("y_audio_latents_norm[:, 1:, :] s ", y_audio_latents_norm[:, 1:, :].shape)
            #print("yhat_audio_latents_norm[:, -1:, :] s ", yhat_audio_latents_norm[:, -1:, :].shape)
        ...
        gen_mels = torch.cat(gen_mels, axis=-1) 
...
create_pred_audio(test_mocap_data, test_audio_data, 60 * mocap_fps, 60 * mocap_fps, "gen_audio.wav")
...
test_mocap_start_times = [10, 50, 100, 150]
test_mocap_duration = 10
    
for test_mocap_start_time in test_mocap_start_times:
    create_ref_anim(test_mocap_data, test_mocap_start_time * mocap_fps, 10 * mocap_fps, "results/anims/orig_mocap_{}.gif".format(test_mocap_start_time))
    ...
    create_pred_audio(test_mocap_data, test_audio_data, test_mocap_start_time * mocap_fps, 10 * mocap_fps, "results/audio/gen_audio_{}-{}_epoch_{}.wav".format(test_mocap_start_time, (test_mocap_start_time + test_mocap_duration), epochs))
...
for test_mocap_start_time in test_mocap_start_times:
    ...
    create_pred_audio(test_mocap_data, test_audio_data, test_mocap_start_time * mocap_fps, 10 * mocap_fps, "results/audio/gen_audio_{}-{}_epoch_{}_test.wav".format(test_mocap_start_time, (test_mocap_start_time + test_mocap_duration), epochs))
